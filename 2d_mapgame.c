...
int main(void) {
    ...
    scanf("%d", &number_pieces);// TODO: Add code to scan in the number of game pieces here.
    ...
    while(i<number_pieces)
    {
        ...
        scanf("%d %d %d", &temp_points, &row, &col);
        if((row==7&&col==0)||(row>=8)||(col>=8)||(row<0)||(col<0)||(temp_points<-9)||(temp_points>9))
         ;
        else if (temp_points<0)
        {
            map[row][col].occupier = MONSTER_TYPE;
            map[row][col].points = temp_points;
        }
        else if (temp_points>0)
        {
